...
generate_cluster_calculations <- function(cbp) {
  ...
  for (i in 1:14) {
    cluster_name <- paste0("fsom", sprintf("%02d", i))
...
# This will create variables cbp_calculations_fsom01 to cbp_calculations_fsom14
...
y_max <- max(cbp_calculations_fsom14$max_mfi)

# Plot
cbp <- ggplot(cbp_calculations_fsom14) +
  geom_hline(
    aes(yintercept = y), 
    data.frame(y = seq(0, y_max, by = y_max / 9.68)),  #change this last number to the highest max_mfi value of the data
    color = "lightgrey"
  ) +
  geom_col(
    aes(
  ...
  coord_polar() +
  labs(title = "Contribution of each feature in cluster fsom 14",
       x = "Feature",
...
cbp1 <- cbp +
  ...
  scale_y_continuous(
    limits = c(-2, 9.8), # Max max_mfi value here
    expand = c(0, 0),
    breaks = c(0, 2, 4, 6, 8)
  ) + 
...
```{r fig.align="center", fig.width=9, fig.height=12, fig.cap="Figure 5: Features' contribution to fsom clusters"}
cbp2 <- cbp1 + 
  # Add labels
  labs(
...
ggsave(filename = "output/SVG/bar_plot_fsom14.svg", plot = cbp2, width = 9, height = 12.6)
```
...
datag <- read_csv("input/data_gate.csv", col_types = cols(event = col_number()))
...
generate_cluster_plots <- function(df, output_dir) {
  ...
  cluster_ids <- paste0("fsom", sprintf("%02d", 1:14))
...
# List of clusters
clusters_to_analyze <- paste0("fsom", sprintf("%02d", 1:14))
