...
+---------------------+--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+--------+
| **Treatment 3**     | Vorinostat (SAHA) (0.5, 0.75 and 1 µM) \| \| HDAC inhibitor, showed to increase H3K9ac (PMID: [28978082](https://pubmed.ncbi.nlm.nih.gov/28978082)). Expected outcome: increased acetylation marks. \| |        |
+---------------------+--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+--------+
| **Replicates**      | 3 biological replicates per experimental group                                                                                                                                                         |        |
+---------------------+--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+--------+
...
# H3K4 methylation marks in a KMT2D LoF model
...
| Treatment 1 | DMSO control                                                                                                                                                                                              |
+-------------+-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| Treatment 2 | CPI-455 HCl (Catalog #S8287) (10, 20, 50 µM) Specific KDM5 inhibitor thus elevating global levels of H3K4me3 (PMID:28432280)                                                                              |
+-------------+-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| Treatment 3 | OICR-9429 (Catalog #S7833) (10, 20, 50 µM) Potent antagonist of the interaction of WDR5 with peptide regions of KMT2 and Histone 3 thus reducing global levels of H3K4 methylation marks (PMID: 26167872) |
+-------------+-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
| Replicates  | 3 biological replicates per experimental group                                                                                                                                                            |
+-------------+-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+
...
### Violin Plots and Statistical Analysis

```{r, fig.height: 9, fig.with: 6}
#| fig.align: center
#| out.width: 100%
#| fig.height: 9
#| fig.with: 6
...
ggsave("output/PNG/genotype_violin_1.png", plot = com1, width = 16, height = 36, units = "cm", dpi = 300, bg = "white")
...
## Drug Treatments to increase or decrease global H3K4 methylation marks
...
```{r, echo=FALSE, warning=FALSE}
# Data summary after scaling
summary_df3 <- summary(df)
# Display summary in HTML using kable
kable(summary_df3, format = "html", caption = "Summary of Data Frame with scaled values")
```
...
#| warning: false
#| 
# Ensure that 'concentration' is a factor with specified levels
df$concentration <- factor(df$concentration, levels = c("0uM", "10uM", "20uM", "50uM"))

# Create a separate subset for DMSO to be used as a control
df_DMSO <- df %>% filter(treatment == "DMSO")

# Split data by genotype and combine DMSO with each treatment for each genotype. With this, DMSO will be plotted as 0uM in each treatment
# For WTC11
df_WT_CPI <- df %>% 
  filter(genotype == "WTC11", treatment == "CPI") %>%
  bind_rows(df_DMSO %>% filter(genotype == "WTC11") %>% mutate(treatment = "CPI"))

df_WT_OICR <- df %>% 
  filter(genotype == "WTC11", treatment == "OICR") %>%
  bind_rows(df_DMSO %>% filter(genotype == "WTC11") %>% mutate(treatment = "OICR"))

# For KMT2D10.4
df_KM_CPI <- df %>% 
  filter(genotype == "KMT2D10.4", treatment == "CPI") %>%
  bind_rows(df_DMSO %>% filter(genotype == "KMT2D10.4") %>% mutate(treatment = "CPI"))

df_KM_OICR <- df %>% 
  filter(genotype == "KMT2D10.4", treatment == "OICR") %>%
  bind_rows(df_DMSO %>% filter(genotype == "KMT2D10.4") %>% mutate(treatment = "OICR"))


```


### CPI treatment in WTC11
...
rp2 <- ggplot(df_WT_CPI, aes(x = "", y = mfi, fill = concentration)) +
  # clouds with thinner density lines
  introdataviz::geom_flat_violin(trim = FALSE, alpha = 0.5,
                                 position = position_nudge(x = rain_height + .05),
                                 size = 0.3) + # Adjust density line thickness
  # rain
  geom_point(aes(color = concentration), shape = 21, size = 1.5, alpha = .3, 
             show.legend = FALSE, position = position_jitter(width = rain_height, height = 0), 
             stroke = 0.5) +
  # boxplots with thinner median line
  geom_boxplot(width = rain_height, alpha = 0.4, show.legend = FALSE, outlier.shape = NA,
               position = position_nudge(x = -rain_height * 2),
               linewidth = 0.5) + # Adjust boxplot line thickness
  # median and IQR with gray outline and fill by concentration
  stat_summary(fun.data = median_iqr, geom = "pointrange", 
               mapping = aes(fill = concentration), shape = 21, size = 0.7, color = "#252525", 
               stroke = 0.8, position = position_nudge(x = rain_height * 3)) +
  # adjust layout
  scale_x_discrete(name = "", expand = c(rain_height * 3, 0, 0, 0.7)) +
  scale_y_continuous(name = "Fluorescence Intensity (Arcsinh Transformation)",
  ...
  facet_wrap(~factor(feature, 
                     levels = c("H3K4me1", "H3K4me2", "H3K4me3"), 
                     labels = c("H3K4me1", "H3K4me2", "H3K4me3")), nrow = 3) +
  scale_fill_manual(values = c("#999999", "#56B4E9", "#009E73", "#E69F00"), name = "CPI Concentration") +
  scale_color_manual(values = c("#999999", "#56B4E9", "#009E73", "#E69F00")) +
  labs(
    title = "Assessment of H3K4 methylation marks in WTC11 iPSC lines treated with CPI",
    subtitle = "CPI is a KDM5 inhibitor, thus elevating global levels of H3K4me3"
  ) +
  theme_minimal() +
  theme(
    panel.grid.major = element_line(color = "lightgray", linewidth = 0.3),
...
ggsave("output/PNG/rainplot_plot_WT_CPI.png", plot = rp2, width = 18, height = 20, units = "cm", dpi = 600, bg = "white")
ggsave("output/SVG/rainplot_plot_WT_CPI.svg", plot = rp2, width = 18, height = 20, units = "cm")

```

### OICR treatment in WTC11
...
rp3 <- ggplot(df_WT_OICR, aes(x = "", y = mfi, fill = concentration)) +
  # clouds with thinner density lines
  introdataviz::geom_flat_violin(trim = FALSE, alpha = 0.5,
                                 position = position_nudge(x = rain_height + .05),
                                 size = 0.3) + # Adjust density line thickness
  ...
  facet_wrap(~factor(feature, 
                     levels = c("H3K4me1", "H3K4me2", "H3K4me3"), 
                     labels = c("H3K4me1", "H3K4me2", "H3K4me3")), nrow = 3) +
  scale_fill_manual(values =  c("#999999", "#0072B2", "#D55E00", "#CC79A7"), name = "OICR Concentration") +
  scale_color_manual(values = c("#999999", "#0072B2", "#D55E00", "#CC79A7")) +
  labs(
    title = "Assessment of H3K4 methylation marks in WTC11 iPSC lines treated with OICR",
    subtitle = "OICR is an antagonist of the interaction of WDR5 with peptide regions of KMT2 and Histone 3 reducing global levels of H3K4 methylation"
  ) +
  theme_minimal() +
  theme(
    panel.grid.major = element_line(color = "lightgray", linewidth = 0.3),
...
ggsave("output/PNG/rainplot_plot_WT_OICR.png", plot = rp3, width = 18, height = 20, units = "cm", dpi = 600, bg = "white")
ggsave("output/SVG/rainplot_plot_WT_OICR.svg", plot = rp3, width = 18, height = 20, units = "cm")

```


### CPI treatment in KMT2D10.4
...
rp4 <- ggplot(df_KM_CPI, aes(x = "", y = mfi, fill = concentration)) +
  # clouds with thinner density lines
  introdataviz::geom_flat_violin(trim = FALSE, alpha = 0.5,
                                 position = position_nudge(x = rain_height + .05),
                                 size = 0.3) + # Adjust density line thickness
  ...
  scale_color_manual(values = c("#999999", "#56B4E9", "#009E73", "#E69F00")) +
  labs(
    title = "Assessment of H3K4 methylation marks in KMT2D10.4 iPSC lines treated with CPI",
    subtitle = "CPI is a KDM5 inhibitor, thus elevating global levels of H3K4me3"
  ) +
  theme_minimal() +
  theme(
...
ggsave("output/PNG/rainplot_plot_KM_CPI.png", plot = rp4, width = 18, height = 20, units = "cm", dpi = 600, bg = "white")
ggsave("output/SVG/rainplot_plot_KM_CPI.svg", plot = rp4, width = 18, height = 20, units = "cm")

```

### OICR treatment in KMT2D10.4
...
rp5 <- ggplot(df_KM_OICR, aes(x = "", y = mfi, fill = concentration)) +
  # clouds with thinner density lines
  introdataviz::geom_flat_violin(trim = FALSE, alpha = 0.5,
                                 position = position_nudge(x = rain_height + .05),
                                 size = 0.3) + # Adjust density line thickness
  ...
  scale_color_manual(values = c("#999999", "#0072B2", "#D55E00", "#CC79A7")) +
  labs(
    title = "Assessment of H3K4 methylation marks in KMT2D10.4 iPSC lines treated with OICR",
    subtitle = "OICR is an antagonist of the interaction of WDR5 with peptide regions of KMT2 and Histone 3 reducing global levels of H3K4 methylation"
  ) +
  theme_minimal() +
  theme(
...
ggsave("output/PNG/rainplot_plot_KM_OICR.png", plot = rp5, width = 18, height = 20, units = "cm", dpi = 600, bg = "white")
ggsave("output/SVG/rainplot_plot_KM_OICR.svg", plot = rp5, width = 18, height = 20, units = "cm")

```
### Statistical analysis in drug treatments per genotype 

```{r}
#| echo: false
#| warning: false

# Create a new column 'condition' by merging 'treatment' and 'concentration'
df$condition <- with(df, paste(treatment, concentration, sep = "_"))

# Convert 'condition' to a factor
df$condition <- factor(df$condition, levels = c("DMSO_0uM", "CPI_10uM", "CPI_20uM", "CPI_50uM", "OICR_10uM", "OICR_20uM","OICR_50uM"))
...
# Fix subsamplint to 5000 events
df_filtered <- df %>%
  group_by(condition, feature, genotype) %>%
  filter(n() >= 5000) %>%  # Keep only groups with at least 5000 events
  slice_sample(n = 5000) %>% # when a category does not have 5000 events, then the condition is dropped. We want to prevent this so check the min number of events per category
  ungroup()

# Summarize the number of events for each feature, genotype, and condition
event_counts <- df %>%
  group_by(feature, genotype, condition) %>%
  summarise(n = n(), .groups = "drop")

# Display the result

event_counts %>%
  kable("html", caption = "Number of Events per Feature, Genotype, and Condition") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"), 
                full_width = FALSE, 
                font_size = 12) %>%
  column_spec(1, bold = TRUE) %>%  # Make feature column bold
  column_spec(4, color = "blue")     # Highlight the count column in red

```



### Assessment of all treatments in WTC11 iPSC line

```{r, warning=FALSE, fig.align='center', out.width='100%', fig.width=6, fig.height=10}

#grouped analysis

vp1 <- df_filtered %>%
  dplyr::filter(genotype == "WTC11") %>% 
  grouped_ggbetweenstats(
    ## arguments relevant for ggbetweenstats
    x = condition,
    y = mfi,
    ...
    palette = "okabeito",
    ## arguments relevant for combine_plots
    annotation.args = list(title = "Assessment of H3K4 methylation marks in WTC11 iPSC lines"),
    plotgrid.args = list(nrow = 3),
     ggplot.component = list(
          scale_color_manual(values =  c("#999999", "#0072B2", "#009E73", "#E69F00", "#D55E00", "#CC79A7", "#F0E442")),
          theme( # Customizations
            text = element_text(family = "Arial", size = 8, color = "black"),
            plot.title = element_text(
              family = "Arial",
...
ggsave("output/PNG/violin_WTC_treatments.png", width = 22, height = 24, units = "cm", dpi = 300)
ggsave("output/SVG/violin_WTC_treatment.svg", plot = vp1, width = 22, height = 24, units = "cm")
...
vp2 <- df_filtered %>%
  dplyr::filter(genotype == "KMT2D10.4") %>% 
  grouped_ggbetweenstats(
    ## arguments relevant for ggbetweenstats
    x = condition,
    y = mfi,
    ...
    palette = "okabeito",
    ## arguments relevant for combine_plots
    annotation.args = list(title = "Assessment of H3K4 methylation marks in KMT2D10.4 iPSC lines"),
    plotgrid.args = list(nrow = 3),
     ggplot.component = list(
          scale_color_manual(values =  c("#999999", "#0072B2", "#009E73", "#E69F00", "#D55E00", "#CC79A7", "#F0E442")),
          theme( # Customizations
...
ggsave("output/PNG/violin_KMT_treatments.png", width = 22, height = 24, units = "cm", dpi = 300)
ggsave("output/SVG/violin_KMT_treatment.svg", plot = vp2, width = 22, height = 24, units = "cm")
```
